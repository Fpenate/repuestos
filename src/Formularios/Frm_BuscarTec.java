...
import Clases.Cls_BuscarTec;
import javax.swing.JOptionPane;
import javax.swing.table.DefaultTableModel;
import javax.swing.table.TableColumnModel;

public class Frm_BuscarTec extends javax.swing.JInternalFrame {

    private final Cls_BuscarTec CP;
    ...
    public Frm_BuscarTec() {
        initComponents();
        CP = new Cls_BuscarTec();
        columnModel = tabla.getColumnModel();
        listar();
    }

    private void listar() {
        tabla.setModel(CP.getDatosTec());
    }
    ...
    private void tablaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tablaMouseClicked
        if (Frm_Entrada.enviar == 1) {
            ...
            Frm_Entrada.txt_nombre.setText(tabla.getValueAt(row, 0).toString());
            Frm_Entrada.txt_badge.setText(tabla.getValueAt(row, 1).toString());
            //Frm_Entrada.txt_cantidad.requestFocus();
            Frm_Entrada.jdc_fecha.requestFocusInWindow();
            Frm_Entrada.enviar = 0;
            dispose();
        } else if (Frm_Salida.enviar == 1) {
            int row = tabla.getSelectedRow();
            Frm_Salida.txt_tecnico.setText(tabla.getValueAt(row, 0).toString());
            //Frm_Salida.txt_descripcion.setText(tabla.getValueAt(row, 1).toString());
            Frm_Salida.jSpinnerMaq.requestFocusInWindow();
            Frm_Salida.enviar = 0;
            dispose();
        }
